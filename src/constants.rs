use super::models::Mode;

/// Searcher: Trades entire pool of assets
/// Listener: Listens to and trades specific pool of assets
pub const MODE: Mode = Mode::Searcher(true, true); // bool = is save results

pub const ASSET_HOLDINGS: [&str; 2] = ["BTC", "USDT"];
pub const FIAT_EXCLUSION: [&str; 13] = ["ARS", "BIDR", "BRL", "EUR", "GBP", "IDRT", "NGN", "PLN", "RON", "RUB", "TRY", "UAH", "ZAR"];
pub const USD_BUDGET: f64 = 50.0; // USD equivalent in each asset holding
pub const MAX_CYCLE_LENGTH: usize = 4;
pub const MIN_ARB_THRESH: f64 = 1.0295; // i.e. 1.015 for 1.5%
